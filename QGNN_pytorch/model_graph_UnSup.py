...
class UnSupOGNN(nn.Module):
    def __init__(self, feature_dim_size, hidden_size, num_GNN_layers, sampled_num, vocab_size, dropout, device):
        ...
        self.sampled_softmax = SampledSoftmax(self.vocab_size, self.sampled_num, self.hidden_size * self.num_GNN_layers,
                                              self.device)
    ...
    def forward(self, Adj_block, X_concat, idx_nodes):
        ...
        input = X_concat
        for layer in range(self.num_GNN_layers):
            input = self.o8gnnlayers[layer](input.double(), Adj_block, True)
